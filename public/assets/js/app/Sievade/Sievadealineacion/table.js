...
$(document).ready(function() {
    ...
    confirmDeleteBtn.onclick = () => {
        if (sievadeIdToDelete) deleteCourse(sievadeIdToDelete);
    };
});
...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/Sievadealineacion/table`,
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function(object) {
                    const finalUrl = `/srh/public/Sievadealineacion/edit/${object.id_alineacion_pnd}`;
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton1">
                                       ...
                                       <!-- Aquí se agrega la opción para eliminar 
                                        <a class="dropdown-item" href="#" onclick="confirmDelete(${object.iid_alineacion_pnd})">
...
function renderTable(response) {
    ...
    if (response.value && response.value.length > 0) {
        response.value.forEach((object) => {
            const finalUrl = `${URL_DEFAULT}/Sievadealineacion/edit/${object.id_crit_princ}`;
            const rowHTML = `
                <tr>
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <!-- Aquí se agrega la opción para eliminar 
                                <a class="dropdown-item" href="#" onclick="confirmDelete(${object.id_crit_princ})">
                                    <span style="background:#6A1B3D" class="icon-container-template">
                                        <div style="text-align: center;">
                                            <i class="fa fa-trash item-icon-menu"></i>
                                        </div>
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/Sievadealineacion/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
            alert('Organización eliminada exitosamente.');
            window.location.href = `${URL_DEFAULT}/Sievadealineacion/list`;
        },
