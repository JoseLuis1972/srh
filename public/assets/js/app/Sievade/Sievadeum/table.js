...
function searchInit() {
    ...
    $.ajax({
        url: URL_DEFAULT.concat('/Sievadeum/table'),
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function(object) {
                    const finalUrl = `/srh/public/Sievadeum/edit/${object.id_unid_medicion}`;
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton1">
                                       ...
                                       <!-- Aquí se agrega la opción para eliminar 
                                        <a class="dropdown-item" href="#" onclick="confirmDelete(${object.id_unid_medicion})">
...
function renderTable(response) {
    ...
    if (response.value && response.value.length > 0) {
        response.value.forEach((object) => {
            const finalUrl = `${URL_DEFAULT}/Sievadeum/edit/${object.id_unid_medicion}`;
            const rowHTML = `
                <tr>
                    <td>
                        <div class="dropdown">
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/Sievadeum/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
            alert('Organización eliminada exitosamente.');
            window.location.href = `${URL_DEFAULT}/Sievadeum/list`;
        },
