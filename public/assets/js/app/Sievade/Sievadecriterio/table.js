...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/Sievadecriterio/table`,
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function(object) {
                    const finalUrl = `/srh/public/Sievadecriterio/edit/${object.id_crit_princ}`;
...
function renderTable(response) {
    ...
    if (response.value && response.value.length > 0) {
        response.value.forEach((object) => {
            const finalUrl = `${URL_DEFAULT}/Sievadecriterio/edit/${object.id_crit_princ}`;
            const rowHTML = `
                <tr>
                    <td>
                        <div class="dropdown">
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/Sievadecriterio/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
            alert('Organización eliminada exitosamente.');
            window.location.href = `${URL_DEFAULT}/Sievadecriterio/list`;
        },
