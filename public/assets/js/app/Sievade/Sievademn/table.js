...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/Sievademn/table`,
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function(object) {
                    const finalUrl = `/srh/public/Sievademn/edit/${object.id_act_mn}`;
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton1">
                                       ...
                                       <!-- Aquí se agrega la opción para eliminar 
                                        <a class="dropdown-item" href="#" onclick="confirmDelete(${object.id_act_mn})">
...
function renderTable(response) {
    ...
    if (response.value && response.value.length > 0) {
        response.value.forEach((object) => {
            const finalUrl = `${URL_DEFAULT}/Sievademn/edit/${object.id_act_mn}`;
            const rowHTML = `
                <tr>
                    <td>
                        <div class="dropdown">
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/Sievademn/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
            alert('Organización eliminada exitosamente.');
            window.location.href = `${URL_DEFAULT}/Sievademn/list`;
        },
