...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/Tablemetasdinamicas/table`,
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function(object) {
                    const finalUrl = `/srh/public/Tablemetasdinamicas/edit/${object.id_metas_ind}`;
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}Tablemetasdinamicas/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
            alert('Organización eliminada exitosamente.');
            window.location.href = `${URL_DEFAULT}/Tablemetasdinamicas/list`;
        },
