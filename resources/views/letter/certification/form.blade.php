                ...
                <div class="col-md-12 grid-margin">
                    <div class="row">
                        <x-template-tittle.tittle-header tittle="Coordinación de Recursos Humanos"
                            caption="Notas Informativas" />
                    </div>
                </div>
            </div>

            <!-- View->modal -->
            @include('letter.informative.modal')

            <!-- SIZE FOR SELECT -->
            <x-template-form.template-form-select-default />
            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card custom-card">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center w-100">
                            <div>
                                <h2 class="card-title" style="margin-bottom: 0;">
                                    {{ isset($item->id_tbl_notas_interno) ? 'Modificar' : 'Agregar ' }}
                                    Nota
                                </h2>
                            </div>
                            <div class="d-flex align-items-center">
                                <!-- Botón 1 -->
                                @if (!isset($item->id_tbl_notas_interno))
                                ...
                                <a href="{{ route('informative.list') }}" class="btn btn-hover-enlarge"
                        ...
                        <div>
                            <form id="myForm" action="{{ route('informative.save') }}" method="POST" class="form-sample">
                                @csrf

                                <!-- item -> hidden -->
                                <x-template-form.template-form-input-hidden name="id_tbl_notas_interno"
                                    value="{{ optional($item)->id_tbl_notas_interno ?? '' }}" />
                                ...
                                <div class="row">
                                    <x-template-form.template-form-select-required :selectValue="$selectDestinatario"
                                        ...
                                        tittle="Destinatario" grid="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12" />
                                </div>
                                ...
                                <div class="row">
                                    <x-template-form.template-form-select-required :selectValue="$selectSolicitante"
                                        ...
                                        tittle="Elaboro" grid="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6" />

                                    <x-template-form.template-form-select-required :selectValue="$selectSolicitante_2"
                                        :selectEdit="$selectSolicitanteEdit" name="id_cat_solicitante_2"
                                        tittle="De" grid="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6" />
                                </div>

                                <x-template-button.button-form-footer routeBack="{{ route('informative.list') }}" />
...
<!-- CODE SCRIPT-->
<script src="{{ asset('assets/js/app/letter/informative/form.js') }}"></script>
<script src="{{ asset('assets/js/app/letter/informative/modal.js') }}"></script>
<script src="{{ asset('assets/js/app/letter/informative/consecutivo.js') }}"></script>
...
<script src="{{ asset('assets/js/app/letter/informative/validate.js') }}"></script>
